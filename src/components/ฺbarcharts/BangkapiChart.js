...
class BangkapiChart extends Component {
    componentDidMount(){
                            ...
                            datasets : [
                                {
                                    ...
                                    data :[0,0,
                                        ...
                                        calculate_avg_of_water_level_new_version(
                                            Array.from(this.props.data)
                                            .filter(item => item.location === "BANGKAPI" && item.date === "07/06/2020")
                                            .map(item => (item.water_level))
                                        )
                                        ,
                                        calculate_avg_of_water_level_new_version(
                                            Array.from(this.props.data)
                                            .filter(item => item.location === "BANGKAPI" && item.date === "08/06/2020")
                                            .map(item => (item.water_level))
                                        )
                                    ]
                                },                            
...
export default connect(mapStateToProps, mapDispatchToProps)(BangkapiChart);